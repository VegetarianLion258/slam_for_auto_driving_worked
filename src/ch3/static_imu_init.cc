...
bool StaticIMUInit::TryInit() {
    ...
    gravity_ = -mean_acce / mean_acce.norm() * options_.gravity_norm_; //归一化到option的重力大小
    ...
    // 检查IMU噪声
    if (cov_gyro_.norm() > options_.max_static_gyro_var) { //0.5
        LOG(ERROR) << "陀螺仪测量噪声太大" << cov_gyro_.norm() << " > " << options_.max_static_gyro_var;
        return false;
    }

    if (cov_acce_.norm() > options_.max_static_acce_var) { //0.05
