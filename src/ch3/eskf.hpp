...
class ESKF {
   public:
    ...
    using Vec18T = Eigen::Matrix<S, 18, 1>;         // 18维向量类型, delta_x
    ...
    using Mat18T = Eigen::Matrix<S, 18, 18>;        // 18维方差类型, 方差18*18
...
bool ESKF<S>::ObserveSE3(const SE3& pose, double trans_noise, double ang_noise) {
    ...
    innov.template head<3>() = (pose.translation() - p_);          // 观测-预测, delta_x之平移部分
